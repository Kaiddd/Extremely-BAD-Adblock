...
(function() {
    ...
    function checkElements()
    {
        ...
        elementsArrayUwU.forEach(el => {
            if (check(el,"advert") || check(el,"affiliate") || check(el,"doubleclick.net") || check(el,"adsense") || check(el,"tracker") || check(el,"interad") || check(el,"interstitial") || (check(el,"google") && check(el,"ad") && !check(el,"captcha") && !check(el,"login")) || check(el,"bncloud") || check(el,"referral") || check(el,"refid") | check(el,"trafficjunky") || check(el,"pb_template") || check(el,"__clb")){
                el.remove();
                counter++;
            }
            ...
            if (el.src && el.src != el.src.replace(/googletagmanager|google-analytics|googleanalytics|googleadservices|adwords|adsense|admob|urchin.com|doubleclick.net/gim,"thisIsAGoogleAd.invalid.url")) {
